  ...
  filterButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      portfolioItems.forEach(item => {
        if (filter === 'all' || item.querySelector('p').textContent.toLowerCase().includes(filter)) {
  ...
  carouselButtons.forEach(button => {
    button.addEventListener('click', function() {
      if (this.classList.contains('prev')) {
        currentSlide = currentSlide > 0 ? currentSlide - 1 : carouselBullets.length - 1;
      } else {
        currentSlide = currentSlide < carouselBullets.length - 1 ? currentSlide + 1 : 0;
      }
      updateCarousel();
    });
  });
...
function updateActiveNavigation() {
  ...
  const navLinks = document.querySelectorAll('header nav ul a, header nav .scond-menu ul a');
  ...
  if (e.altKey && (e.key === 'ArrowUp' || e.key === 'ArrowDown')) {
    ...
    if (currentSection) {
      ...
      if (e.key === 'ArrowDown') {
        ...
        nextIndex = currentIndex - 1 >= 0 ? currentIndex - 1 : sections.length - 1;
      }
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        ...
        if (entry.isIntersecting && !progressBarsAnimated) {
          console.log('Skills section visible - starting progress bar animation');
          animateProgressBars();
